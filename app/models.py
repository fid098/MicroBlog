...
# Association table for the self-referential many-to-many follow relationship
followers = sa.Table(
    'followers',
    db.metadata,
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    def following_posts(self):
        ...
        # User table aliased twice: once as post author, once as follower
        return (
            sa.select(Post)
    ...
    def add_notification(self, name, data):
        # Delete existing notification with same name before inserting new one
    ...
    def revoke_token(self):
        # Set expiration to the past to immediately invalidate
...
class SearchableMixin(object):
    ...
    def search(cls, expression, page, per_page):
        ...
        for i in range(len(ids)):
            when.append((ids[i], i))
        # CASE statement preserves the relevance order returned by Elasticsearch
    ...
    def before_commit(cls, session):
        # Snapshot changes before commit — session.new/dirty/deleted are cleared after commit
        session._changes = {
